...
fun AddressEditScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            // 类型
            Text("类型", style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.primary)
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ADDRESS_TYPES.take(3).forEach { t ->
                    FilterChip(selected = uiState.type == t, onClick = { viewModel.onTypeChange(t) }, label = { Text(t, fontSize = 13.sp) })
                }
            }
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ADDRESS_TYPES.drop(3).forEach { t ->
                    FilterChip(selected = uiState.type == t, onClick = { viewModel.onTypeChange(t) }, label = { Text(t, fontSize = 13.sp) })
                }
            }
            ...
            // 姓名
            OutlinedTextField(uiState.name, { viewModel.onNameChange(it) }, label = { Text("姓名（可选）") }, placeholder = { Text("收件人姓名") }, singleLine = true, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp))

            // 电话
            OutlinedTextField(uiState.phone, { viewModel.onPhoneChange(it) }, label = { Text("电话（可选）") }, placeholder = { Text("手机号码") }, singleLine = true, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Phone), modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp))
            ...
            Button(
                ...
                enabled = !uiState.isWorking && uiState.isDirty && uiState.title.isNotBlank() && uiState.address.isNotBlank()
            ) {
